...
;
	...
	{
		$k = UserInfo::model ()->findByAttributes ( array (
		...
		if (isset($_POST['UserInfo'])) {
			...
			$model->username = UserController::getnamefromid(Yii::app()->user->getid());
			if ($model->save()) {
				$this->redirect(array('view','id'=>$model->iduser_info));
			}
		}
	...
	throw new CHttpException ( 400, 'One is enough. Please do not repeat this request again.' );
	}
	...
	public function getidfrom_userid($id){
		$userinfoid = UserInfo::model()->findByAttributes(array('user_id'=>$id))->iduser_info;
		return $userinfoid;
	}
	/**
